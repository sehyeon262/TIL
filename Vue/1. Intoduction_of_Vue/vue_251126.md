...
## Client Side Rendering 정의
- <span style="color:blue">**클라이언트에서 콘텐츠를 렌더링 하는 방식**</span>
...
- <span style="color:red">**재사용 가능한 코드 블록**</span>
...
<script>
    const { createApp, ref } = Vue  // 2번: 구조 분해 할당
...
<div id="app">
     <h1>{{ message }}</h1>  <!-- HTML에서는 변수명으로 값 표시 -->
</div>
```
...
const app = createApp({
    setup() {
        ...
        // HTML에서 사용하기 위해선 리턴 반드시 작성 !!! (변수, 함수)
        return {
            message
        }
    }
