...
# True(1) ==> 연결되어있다. x에서 y로 가는 간선이 존재 adj[x][y] = Trie // adj[x][y] = 1
# False(0) ==> 연결이 안 되어씽다. x에서 y로 가는 간선이 없다. adj[x][y] = False // ADJ[X][Y] = 0
...
for i in range(E):
    ...
    # s정점에서 e정점으로 가는 간선이 있다 (무향 그래프의경우임, 유향 그래프면 두번째줄 쓰면 안됨)
    adj[s][e] = 1
    adj[e][s] = 1
#
# for i in range(V+1):
...
def DFS(s, N):
    ...
    while True:
        ...
        for nv in range(1, V+1):
            ...
            if adj[v][nv] == 1 and not visited[nv]:
            # if (adj[v][nv] == 1) and (visited[nv] == 0) (위에랑 같은 말)
